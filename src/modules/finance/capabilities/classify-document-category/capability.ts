...
function buildSystemPrompt(): string {
	...
	return [
		...
		'',
		'Guidance:',
		'- A bill from a supplier to us = expense.sales_cost.invoice (project cost) or an opex.* category.',
		'- A bill we issue to a customer = revenue.invoice_out.',
		'- A store/restaurant/transport receipt = the matching expense.opex.* category.',
		'- Purchase orders / contracts / quotations = the document_only.* categories.',
		'',
