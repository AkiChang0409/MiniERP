<script lang="ts">
	...
	const flow = [
		{ href: '/sales-crm/quotations', label: 'Quotations' },
		{ href: '/sales-crm/orders', label: 'Sales Orders' }
	];
	const utilities = [
		{ href: '/sales-crm', label: 'Overview' },
		{ href: '/sales-crm/customers', label: 'Customers' }
	];

	function isActive(href: string, pathname: string) {
		if (href === '/sales-crm') return pathname === '/sales-crm';
...
<nav class="mb-6 rounded-2xl border border-slate-200 bg-white px-5 py-4 shadow-sm">
	<div class="mb-4 flex items-center gap-1 text-xs">
		...
		{/each}
		<span class="ml-3 text-[10px] uppercase tracking-[0.18em] text-slate-400">Quote to Cash</span>
	</div>
