...
export const load: PageServerLoad = async (event) => {
	if (!event.platform) {
		return { purchaseOrders: [], suppliers: [], inventoryItems: [], warehouses: [], bins: [] };
	}
	const ctx = await createModuleContext(event);
	const procurement = createProcurementApi(ctx);
	const inventory = createInventoryApi(ctx);
	const [purchaseOrders, suppliers, inventoryItems, warehouses] = await Promise.all([
	...
	return { purchaseOrders, suppliers, inventoryItems, warehouses, bins: binLists.flat() };
};
...
export const actions: Actions = {
	createPurchaseOrder: async (event) => {
		...
		const ctx = await createModuleContext(event);
		const procurement = createProcurementApi(ctx);
		const created = await procurement.createPurchaseOrder({
		...
		redirect(303, `/procurement/purchase-orders/${created.id}`);
	},
